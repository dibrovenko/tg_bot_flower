...
async def get_courier_info(claim_id: str) -> dict:
    py_logger.debug("confirmation_order")
    load_dotenv(find_dotenv())
    api_key = os.getenv('api_key_yandex_delivery')

    url = f"https://b2b.taxi.yandex.net/b2b/cargo/integration/v2/driver-voiceforwarding"
    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=ssl_context)) as session:
            async with session.post(url, headers=headers, json=data) as response:
                ...
                try:
                    return {"result": True,
                            "phone": f"{res_json['phone']}, доб. {res_json['ext']}"}
                except:
                    return {"result": True,
